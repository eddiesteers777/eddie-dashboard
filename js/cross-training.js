/* ==========================================
   ...
   them as global variables from a classic
   <script> tag (that never worked: marathonData.js
   uses `export`, which a classic script can't
   parse at all).
========================================== */
...
// Cloud sync is a nice-to-have, not a single point of failure — if it
// can't load for any reason, the page still initializes normally from
// whatever is already saved in localStorage.
import("./cloudSync.js")
    .then(({ initCloudSync }) => initCloudSync().then(start).catch(start))
    .catch(() => start());
