...
- [ ] **Publish the updated rules (adds `coachingPlans` + `coachingPlanDrafts` for coaching Phase A; also includes `clientRecords`, `coachNotes`, `clientUpdates`).** Until then profiles, notes and updates can't be read or saved; the app degrades quietly (no intake nag, profile save says "isn't switched on yet", the Notes tab says the rules may not be published).
